-- ============================================================
-- Improve Samsara unit_id extraction.
--
-- Customer's Samsara messages use the format
--   **🚨 Vehicle Alert - Unit 7 - S007**
-- where the FIRST token is the Samsara internal fleet number ('7')
-- and the SECOND is the TMS code ('S007') that lives in
-- driver_unit_assignments. The previous regex grabbed the first
-- token, which never matched the TMS code → most rows came back
-- unresolved.
--
-- Fix: extract BOTH candidates and try each in turn. If the TMS
-- code resolves, use it; otherwise fall back to the Samsara
-- single-token extraction. No regressions for messages that only
-- have one token (e.g. "Vehicle Alert - Unit M82**") because the
-- TMS extractor returns NULL there and the Samsara extractor
-- handles it as before.
-- ============================================================

-- Single-token capture: "Vehicle Alert - Unit X" or "Unit: X" header line
CREATE OR REPLACE FUNCTION public.extract_samsara_unit_id(p_text text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(
    (regexp_match(p_text, 'Vehicle Alert\s*-\s*Unit\s+([A-Z0-9]*\d[A-Z0-9]*)', 'i'))[1],
    (regexp_match(p_text, 'Unit:?\s*\*?\*?\s*([A-Z0-9]*\d[A-Z0-9]*)', 'i'))[1]
  );
$$;

-- Second-token capture: "Vehicle Alert - Unit X - Y**" → returns Y (the TMS code).
-- Anchored to the closing `**` markdown bold marker (with optional spaces) so
-- a stray "Unit 5018" inside an SPN line later in the message isn't captured.
-- Returns NULL when there's no second hyphen-separated token.
CREATE OR REPLACE FUNCTION public.extract_tms_unit_id(p_text text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT (regexp_match(
    p_text,
    'Vehicle Alert\s*-\s*Unit\s+[A-Z0-9]+[ \t]*-[ \t]*([A-Z0-9]*\d[A-Z0-9]*)[ \t]*\*\*',
    'i'
  ))[1];
$$;

-- Resolve a driver from a message: try the TMS code first (preferred — matches
-- the customer's assignments file), fall back to the Samsara fleet number.
-- Returns the resolved (driver_id, driver_name) row or empty if neither hits.
CREATE OR REPLACE FUNCTION public.resolve_driver_for_message(
  p_text text,
  p_ts   timestamptz
)
RETURNS TABLE (driver_id text, driver_name text)
LANGUAGE sql
STABLE
AS $$
  WITH candidates AS (
  ...
  SELECT r.driver_id, r.driver_name
  FROM candidates c
...
GRANT EXECUTE ON FUNCTION public.extract_samsara_unit_id(text)              TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.extract_tms_unit_id(text)                  TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.resolve_driver_for_message(text, timestamptz) TO anon, authenticated, service_role;

-- ============================================================
-- Update the four Samsara RPCs to use the dual resolver.
-- Return shapes unchanged → CREATE OR REPLACE is sufficient.
-- The canonical unit_id is now COALESCE(TMS, Samsara) so the
-- displayed unit identifier matches the TMS file when available.
-- ============================================================

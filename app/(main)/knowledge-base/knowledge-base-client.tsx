...
export function KnowledgeBaseClient({ initialRules }: Props) {
  ...
  function clearAll() {
    ...
    setActiveFilter('all')
    setEscalationFilter(new Set())
    setSearchQuery('')
  }

  /* Domain options with counts */
  const domainOptions = useMemo(() =>
    allDomains.map(d => ({
      value: d,
      label: formatDomain(d),
      count: rules.filter(r => r.domain === d).length,
    }))
  , [allDomains, rules])
  ...
  const escalationOptions = useMemo(() =>
    allEscalationDepts.map(d => ({
      ...
      count: rules.filter(r => r.escalation_path.includes(d)).length,
    }))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="grid grid-cols-4 gap-3">
        <StatCard label="Total Rules"    value={totalRules}    icon={BookOpen}      color="var(--accent)" />
        <StatCard label="Active Rules"   value={activeRules}   icon={Activity}      color="var(--severity-low)" />
        <StatCard label="Critical Rules" value={criticalRules} icon={AlertCircle}   color="var(--severity-critical)" />
        <StatCard label="Violations Today" value={0}           icon={Shield}        color="var(--text-muted)" placeholder />
      </div>
      ...
      >
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <MultiSelectDropdown label="Domain"     options={domainOptions}     selected={domainFilter}     onChange={setDomainFilter} />
          <MultiSelectDropdown label="Severity"   options={severityOptions}   selected={severityFilter}   onChange={setSeverityFilter} />
          <MultiSelectDropdown label="Escalation" options={escalationOptions} selected={escalationFilter} onChange={setEscalationFilter} />

          {/* Active toggle */}
          <div className="flex items-center" style={{ border: '1px solid var(--border-subtle)', borderRadius: 8, overflow: 'hidden', flexShrink: 0 }}>
            {(['all', 'active', 'inactive'] as const).map(v => (
              <button
                ...
                style={{
                  ...
                  background: activeFilter === v ? 'var(--accent-dim)' : 'var(--bg-card)',
                  color: activeFilter === v ? 'var(--accent)' : 'var(--text-muted)',
                  borderRight: v !== 'inactive' ? '1px solid var(--border-subtle)' : 'none',
        ...
        {hasFilters && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {[...domainFilter].map(d => (
              <FilterPill key={`domain:${d}`} label={formatDomain(d)} onRemove={() => {
                const next = new Set(domainFilter); next.delete(d); setDomainFilter(next)
              }} />
            ))}
            {[...severityFilter].map(s => (
              <FilterPill key={`sev:${s}`} label={s.charAt(0).toUpperCase() + s.slice(1)} onRemove={() => {
                const next = new Set(severityFilter); next.delete(s); setSeverityFilter(next)
              }} />
            ))}
            {[...escalationFilter].map(e => (
              <FilterPill key={`esc:${e}`} label={e.charAt(0).toUpperCase() + e.slice(1)} onRemove={() => {
                const next = new Set(escalationFilter); next.delete(e); setEscalationFilter(next)
              }} />
            ))}
            {activeFilter !== 'all' && (
      ...
      {rulesByDomain.size === 0 ? (
        <div style={{ padding: '48px 20px', textAlign: 'center', background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 12 }}>
          ...
          {hasFilters && (
            <button onClick={clearAll} style={{ marginTop: 12, fontSize: 12, color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer' }}>
              Clear filters
            </button>
          )}
        </div>
...
function StatCard({ label, value, icon: Icon, color, placeholder }: {
  label:       string
  value:       number
    ...
    <div style={{
      ...
      >
        ...
        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
          {activeCount} / {totalInDomain} active
        </span>
        {critCount > 0 && (
          <span style={{
            display: 'flex', alignItems: 'center', gap: 4,
            fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 20,
            background: 'rgba(248,81,73,0.12)', color: 'var(--severity-critical)',
          }}>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid var(--border-subtle)' }}>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: '88px auto 1fr auto',
            padding: '5px 16px',
            background: 'var(--bg-surface)',
            gap: 12,
          }}>
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 10, color: 'var(--text-muted)' }}>
          ...
          {(rule.related_rules?.length ?? 0) > 0 && (
            <span>{rule.related_rules!.length} related</span>
          )}
      ...
      >
        <IconBtn icon={Eye}    title="View details"  onClick={() => onView(rule)} />
        <IconBtn icon={Pencil} title="Edit rule"     onClick={() => onEdit(rule)} />
        <IconBtn
  ...
  return (
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      >
        {label}
        {count > 0 && (
          <span style={{ background: 'var(--accent)', color: 'var(--bg-base)', borderRadius: 10, padding: '0px 5px', fontSize: 10, fontWeight: 800 }}>
            {count}
          </span>
        )}
...
function IconBtn({ icon: Icon, title, onClick, color }: {
  icon:    React.ElementType
  title:   string
  onClick: () => void
  color?:  string
  ...
  return (
    <button
      ...
      style={{
        ...
        background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)',
        borderRadius: 6, cursor: 'pointer', color: color ?? 'var(--text-secondary)',
      }}

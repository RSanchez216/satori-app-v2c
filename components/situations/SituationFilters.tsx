...
export type StatusFilter = 'all' | 'open' | 'kb_flagged' | 'pending' | 'resolved'
export type DeptFilter   = string   // dynamic — 'All' or any department name

const STATUS_CHIPS: { id: StatusFilter; label: string }[] = [
  { id: 'all',        label: 'All'        },
  { id: 'open',       label: 'Open'       },
  { id: 'kb_flagged', label: 'KB Flagged' },
  { id: 'pending',    label: 'Monitoring' },
...
interface Props {
  status:      StatusFilter
  dept:        DeptFilter
  depts:       string[]                           // dynamic list from real data
  totalCounts: Partial<Record<StatusFilter, number>>
  onStatus:    (s: StatusFilter) => void
  onDept:      (d: DeptFilter) => void
}

export function SituationFilters({ status, dept, depts, totalCounts, onStatus, onDept }: Props) {
  const deptChips = ['All', ...depts]
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {STATUS_CHIPS.map((chip) => {
          ...
          return (
            ...
            >
              {chip.id === 'kb_flagged' && (
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: 'var(--kb-purple)' }} />
              )}
              {chip.label}
              {count !== undefined && (
                <span
      ...
      {/* Dept chips — dynamic from real data */}
      {deptChips.length > 1 && (
        <div className="flex items-center gap-1 flex-wrap">
          {deptChips.map((d) => {
            const active = dept === d
